...
@export var PlayerCharacterMesh: Node3D
...
func _physics_process(delta):
	...
	var mesh_front = PlayerCharacterMesh.global_transform.basis.z
	...
	if $control_stay_delay.is_stopped():
		#FOLLOW CAMERA
		$h.rotation.y = lerp_angle($h.rotation.y, PlayerCharacterMesh.global_transform.basis.get_euler().y, delta * auto_rotate_speed)
